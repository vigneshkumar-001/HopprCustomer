// Package delivery trust — central status→style mapper (Customer App).
//
// Single source of truth for how a `parcelStatus` value renders anywhere in
// the package-tracking screen: label, one-line message, icon, and color.
// Widgets must read colors from here instead of hardcoding hex values, so a
// palette change never needs a multi-file hunt. Mirrors the driver app's
// mapper of the same name — kept as a separate file since the two apps
...
/// The 6 forward states in canonical order. `FAILED_DELIVERY` is a branch,
/// not a forward step, and is intentionally excluded — callers needing
/// timeline position should special-case it.
...
// duplicated per-widget (teal/indigo/amber have no existing equivalent in
// Core/Consents/app_colors.dart).
  ...
  switch (status) {
    ...
    case 'PICKED_UP':
      return const PackageStatusStyle(
        ...
        message: 'Your package has been verified and collected by the courier.',
    ...
    case 'IN_TRANSIT':
      return const PackageStatusStyle(
        ...
        message: 'Your package is moving toward the delivery address.',
    ...
    case 'OUT_FOR_DELIVERY':
      return const PackageStatusStyle(
        ...
        message: 'The courier is approaching the receiver.',
    ...
    case 'FAILED_DELIVERY':
      return const PackageStatusStyle(
        ...
        message: 'The courier could not complete the delivery.',
    ...
    default:
      return const PackageStatusStyle(
        label: 'Your courier is confirmed',
        message: 'We are matching you with a nearby courier.',
